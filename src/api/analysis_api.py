...
async def Generate_llm_response(db: AsyncSession = Depends(get_db)):
    ...
    logger.info("--- Starting Generate LLM Response Batch ---")
    
    # Move semaphore inside the endpoint to bind to the active event loop
    llm_semaphore = asyncio.Semaphore(MAX_CONCURRENT_LLM_CALLS)
    ...
    logger.info(f"Found {len(pending_surveys)} surveys to process. Extracting data...")

    # EXTRACT DATA to pure Python dictionaries to prevent DetachedInstanceError
    processing_batch = []
    for survey in pending_surveys:
        processing_batch.append({
            "survey_id": survey.id,
            "employee_id": survey.employee_id,
            "raw_answers": survey.raw_answers
        })

    # Pass the isolated DICTIONARY (item) instead of the DB object
    async def process_single_llm_call(item):
        async with llm_semaphore:
            if not item["raw_answers"]:
                logger.warning(f"No raw answers found for Survey ID {item['survey_id']}.")
                return item, None
            ...
            try:
                logger.info(f"Sending Survey ID {item['survey_id']} (Emp ID: {item['employee_id']}) to LLM...")
                llm_result = await getting_analysis_from_llm(data_str)
                logger.info(f"Successfully received LLM response for Survey ID {item['survey_id']}.")
                return item, llm_result
            except Exception as e:
                logger.error(f"LLM Error for Emp ID {item['employee_id']}: {e}", exc_info=True)
                return item, None

    # Use gather() to wait for ALL network calls to finish before touching DB
    logger.info("Dispatching concurrent LLM tasks...")
    tasks = [process_single_llm_call(item) for item in processing_batch]
    results = await asyncio.gather(*tasks)
    
    processed_count = 0
    failed_count = 0

    # Create a lookup dictionary to safely map results back to DB objects
    survey_lookup = {s.id: s for s in pending_surveys}

    logger.info("Processing LLM results and preparing database updates...")
    
    for item, analysis_json in results:
        survey = survey_lookup.get(item["survey_id"])
        if not survey: 
            continue
        ...
        if analysis_json:
            try:
                ...
                try:
                    ...
                    logger.error(f"Sentiment Calc Error for Emp ID {survey.employee_id}: {calc_err}")
                    sentiment_val = "Analyzed" 
                ...
                logger.error(f"Data mapping error for Emp ID {survey.employee_id}: {db_err}", exc_info=True)
                survey.status = "FAILED"
                failed_count += 1
        else:
            # Handle LLM Failure
            logger.warning(f"LLM returned None for Survey ID {survey.id}. Marking as FAILED.")
            survey.status = "FAILED"
            failed_count += 1

    # COMMIT ONCE AT THE VERY END
    # This completely eliminates the Expiration/DetachedInstanceError traps.
    logger.info("Committing all transactions to the database...")
    try:
        await db.commit()
        end_time = time.time()
        
        # 3. Calculate and format the elapsed time nicely (Minutes and Seconds)
        elapsed_seconds = end_time - start_time
        minutes, seconds = divmod(elapsed_seconds, 60)
        time_str = f"{int(minutes)}m {int(seconds)}s"
        
        logger.info(f"--- Batch Complete: {processed_count} Processed | {failed_count} Failed ---")
        logger.info(f"Total execution time: {time_str}") # <--- Log the time
    except Exception as commit_err:
        logger.critical(f"Final DB Commit Error: {commit_err}", exc_info=True)
        await db.rollback()
